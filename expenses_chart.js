...
function addExpenseToChart(name, amount) {
    ...
    // TODO add expense to chart (done)
...
addExpenseButton.addEventListener('click', function() {
    ...
    // TODO call function to update chart (done)
...
// TODO add event listener to click the Add Expense button when the enter key is pressed (done)
...
window.addEventListener('keyup', function () {
    if (event.keyCode === 13) {
        ...
        if ( inputElements.includes(document.activeElement) ) {
            ...
            expenseNameInput.focus() // brings focus back to expense type once value is entered to chart.
        }
    }
})